...
function halloween_store_init() {
    ...
    $args = array(
        ...
        //this is the function that actually registers  the custom post type. by passing the $args array to the register_post_type() function
        //and passing the post type name as the first parameter
...
// Action hook to create a shortcode for displaying products
add_shortcode( 'halloween_products', 'halloween_store_display_products' );

function halloween_store_display_products( $atts ) {
    ...
    if ( $query->have_posts() ) {
        echo '<div class="halloween-products-list">';
        while ( $query->have_posts() ) {
            $query->the_post();
            $hween_product_data = get_post_meta( get_the_ID(), '_halloween_product_data', true );
            $price = !empty($hween_product_data['price']) ? $hween_product_data['price'] : 'N/A';
            $image = get_the_post_thumbnail( get_the_ID(), 'medium' );

            echo '<div class="halloween-product">';
            ...
            echo '<a href="' . get_permalink() . '" class="view-product">View Details</a>';
            echo '</div>';
        }
        echo '</div>';
    } else {
        echo '<p>No Halloween products found.</p>';
    }
